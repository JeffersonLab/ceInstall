...
filename="geant4/geant4-v$G4_VERSION.tar.gz" # includes subdirectory name

# tarball name is different in old geant4 version
if [[ $PHYS_VERSION == 2.4* || $PHYS_VERSION == 2.5* ]]; then
    filename="geant4/geant$G4_VERSION.tar.gz"
    tar_strip=1
fi
...
# for linux, add entries to cmake_options
if [[ $OSTYPE == linux* ]]; then
    opt1="-I$QTDIR/include \
    ...
    opt2=' -W -Wall -pedantic -Wno-non-virtual-dtor -Wno-long-long -Wwrite-strings -Wpointer-arith -Woverloaded-virtual -pipe'
    cmake_options="$cmake_options $opt1 $opt2"
fi
...
case $(uname -s) in
    Linux)
	  ...
	  opt1=" -I$QTDIR/include -I$QTDIR/include/QtOpenGL -I$QTDIR/include/QtWidgets"
		...
		opt3=" -W -Wall -pedantic -Wno-non-virtual-dtor -Wno-long-long -Wwrite-strings -Wpointer-arith -Woverloaded-virtual -pipe"
